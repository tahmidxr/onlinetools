...
let totalPages = 0;
let selectedImages = []; // For multiple image to PDF conversion
...
function generateToolInterface(toolType) {
    const toolConfigs = {
        ...
        'images-to-pdf': { title: 'Multiple Images to PDF', icon: 'fa-images', accept: '.jpg,.jpeg,.png', formats: 'JPEG, PNG', multiple: true },
    ...
    if (toolType === 'image-resizer') {
        ...
        `;
    } else if (toolType === 'images-to-pdf') {
        optionsHTML = `
            <div class="options-panel" id="options-panel" style="display: none;">
                <h4><i class="fas fa-cog"></i> PDF Creation Options</h4>
                <div class="option-group">
                ...
                <div class="option-group">
                    ...
                    <select id="image-fit">
                        <option value="contain" selected>Fit to Page (Maintain Ratio)</option>
                        <option value="cover">Fill Page (May Crop)</option>
                        <option value="stretch">Stretch to Page</option>
                    </select>
                </div>
                <div class="images-list" id="images-list" style="margin-top: 1rem;">
    ...
    // Different upload area for multiple images
    let uploadAreaHTML = '';
    if (toolType === 'images-to-pdf') {
        uploadAreaHTML = `
            <div class="upload-area" id="upload-area">
                <i class="fas fa-images"></i>
                <p><strong>Select multiple images to create PDF</strong></p>
                <p>You can select JPG and PNG images together</p>
                <button class="upload-btn" id="browse-btn">
                    <i class="fas fa-folder-open"></i> Select Images
                </button>
                <p style="font-size: 0.9rem; color: #999; margin-top: 1rem;">Maximum total size: 100MB</p>
                <input type="file" id="file-input" class="file-input" accept="${config.accept}" multiple>
            </div>
        `;
    } else {
        uploadAreaHTML = `
    ...
    return `
        <div class="tool-interface">
            ...
            <button class="convert-btn" id="convert-btn" disabled>
                <i class="fas fa-sync-alt"></i> ${toolType === 'images-to-pdf' ? 'Create PDF' : 'Convert File'}
            </button>
            ...
            <div class="result-area" id="result-area" style="display: none;">
                ...
                <button class="new-file-btn" id="new-file-btn">
                    <i class="fas fa-file-upload"></i> ${toolType === 'images-to-pdf' ? 'Create Another PDF' : 'Convert Another File'}
                </button>
            </div>
        </div>
    `;
...
function initializeToolListeners(toolType) {
    ...
    if (toolType === 'images-to-pdf') {
        freshFileInput.addEventListener('change', handleMultipleFileSelect);
    } else {
...
function handleMultipleFileSelect(e) {
    ...
    if (totalSize > maxSize) {
        alert('Total file size exceeds 100MB. Please select fewer or smaller images.');
        return;
    }
...
function updateMultipleImagesDisplay() {
    ...
    uploadArea.innerHTML = `
        ...
        <p><strong>${selectedImages.length} image(s) selected</strong></p>
        ...
        <button class="upload-btn" id="browse-btn" style="margin-top: 1rem;">
            <i class="fas fa-plus-circle"></i> Select Different Images
        </button>
        <input type="file" id="file-input" class="file-input" accept=".jpg,.jpeg,.png" multiple>
    `;

    // Display images list
    if (imagesList) {
        imagesList.innerHTML = '<h4 style="margin-bottom: 0.5rem;">Selected Images:</h4>';
        const listContainer = document.createElement('div');
        listContainer.style.cssText = 'max-height: 200px; overflow-y: auto; border: 1px solid #ddd; border-radius: 8px; padding: 0.5rem;';

        selectedImages.forEach((file, index) => {
            const item = document.createElement('div');
            item.style.cssText = 'display: flex; align-items: center; justify-content: space-between; padding: 0.5rem; background: #f8f9fa; margin-bottom: 0.5rem; border-radius: 4px;';

            const fileInfo = document.createElement('div');
            fileInfo.innerHTML = `
                <strong>${index + 1}. ${file.name}</strong><br>
                <small style="color: #666;">${formatFileSize(file.size)} • ${file.type.split('/')[1].toUpperCase()}</small>
            `;

            const removeBtn = document.createElement('button');
            removeBtn.innerHTML = '<i class="fas fa-times"></i>';
            removeBtn.style.cssText = 'background: #dc3545; color: white; border: none; padding: 0.25rem 0.5rem; border-radius: 4px; cursor: pointer;';
        ...
        imagesList.appendChild(listContainer);
    }

    // Re-initialize listeners
    setTimeout(() => {
        initializeToolListeners('images-to-pdf');
    }, 100);
}
...
function resetTool() {
    ...
    const isMultipleImages = currentTool === 'images-to-pdf';
    ...
    if (convertBtn) {
        ...
        convertBtn.innerHTML = '<i class="fas fa-sync-alt"></i> ' + (isMultipleImages ? 'Create PDF' : 'Convert File');
    }
...
async function processFile(toolType) {
    if (!uploadedFile && selectedImages.length === 0) {
    ...
    if (convertBtn) {
        convertBtn.disabled = true;
        convertBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ' + (toolType === 'images-to-pdf' ? 'Creating PDF...' : 'Converting...');
    }
    ...
    try {
        ...
        switch(toolType) {
            ...
            case 'png-to-pdf':
                ...
                break;
            case 'images-to-pdf':
                result = await convertMultipleImagesToPdf(selectedImages);
                break;
            case 'word-to-pdf':
        ...
        if (convertBtn) {
            convertBtn.disabled = false;
            convertBtn.innerHTML = '<i class="fas fa-sync-alt"></i> ' + (toolType === 'images-to-pdf' ? 'Create PDF' : 'Convert File');
        }
    }
}

// Convert multiple images to PDF
async function convertMultipleImagesToPdf(images) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const fitMode = fitSelect?.value || 'contain';
            ...
            const fileName = 'combined_images.pdf';
